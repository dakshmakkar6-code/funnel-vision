...
def generate_teardown_report(image_path: str | list[str], bounding_boxes: list[dict[str, Any]] | list[Any], flow_analysis: dict[str, Any]) -> str:
    ...
    with TemporaryDirectory() as tmp_dir:
        ...
        for page_index, image_file in enumerate(image_files, start=1):
            ...
            for box in normalized_boxes:
                ...
                x_val = float(str(x))
                y_val = float(str(y))
                width_val = float(str(width))
                height_val = float(str(height))
                if y_val + height_val <= segment_top or y_val >= segment_bottom:
                    continue

                local_y = float(y_val) - float(segment_top)
                draw.rectangle(
